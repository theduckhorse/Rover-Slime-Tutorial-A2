...
static int PErrorEndL(void)
{
    ...
    int pulsetime2=0;// the time between two notches
...
void disableWEncoderInterrupt(void){
    GPIO_disableInterrupt(GPIO_PORT_P2, GPIO_PIN6);// disable interrupts for the wheel encoder
    GPIO_disableInterrupt(GPIO_PORT_P2, GPIO_PIN7);
}
void enableWEncoderInterrupt(void){
    GPIO_enableInterrupt(GPIO_PORT_P2, GPIO_PIN6);// enable interrupts for the wheel encoder
